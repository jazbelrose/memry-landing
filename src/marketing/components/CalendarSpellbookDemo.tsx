...
export function CalendarSpellbookDemo() {
  ...
  const renderSidebar = () => {
    const icons = [
      ...
      { y: 188, active: false },
      { y: 228, active: false },
      { y: 276, active: true },  // calendar active
      { y: 316, active: false },
      { y: 364, active: false },
    ];
    return (
      <g>
        ...
        {/* + button */}
        <rect x={8} y={42} width={32} height={32} rx={10} fill={C.accent} />
        <text x={SB_W / 2} y={64} textAnchor="middle" fill="#fff" fontSize={18} fontFamily={FONT}>+</text>
        {icons.map((ic, i) => (
          <g key={i}>
            <rect
